...
class OrderController extends Controller
{
    ...
    public function index(OrderItem $order_items, Request $request)
    {
        if (!Auth::user()->role_as == '3') {
            ...
            return view('manager.orders.index', compact('order_items', 'ajuan_order_items', 'marikina_order_items', 'ortigas_order_items'));
        }
    }
        ...
        foreach ($request->addmore as $key => $value) {
            ...
            if ($product) {
                ...
                if ($value['quantity'] <= $product->quantity) {
                    ...
                    $order_item->sku = strtoupper(str_replace(' ', '', $value['sku']));
                ...
                return redirect('/manager/orders')->with('error', 'No item exists!');
            }
        }

        return redirect('/manager/orders')->with('message', 'Delivery Created Successfully');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect('/manager/orders')->with('message', 'Delivery item updated successfully');
    }
    ...
    public function generateForm(Request $request, OrderItem $orderItem)
    {
        ...
        if (OrderItem::query()->whereEquals('drnumber', $validated)->exists()) {
            ...
            $drnumber = $validated;
            $pdf = PDF::loadView('manager.orders.form', compact('order_items', 'drnumber'))->setPaper('A4', 'portrait');
