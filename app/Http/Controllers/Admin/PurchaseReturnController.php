...
class PurchaseReturnController extends Controller
{
    public function index(PurchaseReturn $purchase_returns, Request $request)
    {
        ...
        switch ($role) {
            ...
            default:
                ...
                return view('admin.purchasereturns.index', compact('purchase_returns', 'ajuan_purchase_returns', 'marikina_purchase_returns', 'ortigas_purchase_returns'));
                break;
        }
    }
        ...
        foreach ($request->addmore as $key => $value) {
            ...
            if ($product) {
                ...
                if ($value['quantity'] <= $product->quantity) {
                    ...
                    $purchase_returns->sku = $value['sku'];
                ...
                return redirect('/admin/purchasereturns')->with('error', 'No item exists!');
            }
        }

        return redirect('/admin/purchasereturns')->with('message', 'Purchase Return Created Successfully');
    ...
    public function destroy($id)
    {
        ...
        if ($purchase_returns){
            ...
            return redirect('/admin/purchasereturns')->with('warning', 'Purchase return item deleted!');
        } else{
    ...
    {
        $purchase_returns = PurchaseReturn::findOrFail($id);
        return view('admin.purchasereturns.edit', compact('purchase_returns'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect('/admin/purchasereturns')->with('message', 'Purchase return item updated successfully');
    }
    ...
    public function generateForm(Request $request, PurchaseReturn $PurchaseReturn)
    {
        ...
        if (PurchaseReturn::query()->whereEquals('drnumber', $validated)->exists()) {
            ...
            $pdf = PDF::loadView('admin.purchasereturns.form', compact('purchase_returns','drnumber'))->setPaper('A4', 'portrait');
            return $pdf->download('purchasereturns-'.$todayDate.'.pdf');
        } else
